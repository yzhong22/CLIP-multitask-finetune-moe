...
def mixup_target_multilabel(target, num_classes, lam=1.0, smoothing=0.0):
    off_value = smoothing / num_classes
    on_value = 1.0 - smoothing + off_value
    y1 = target
    y2 = target.flip(0)
...
class Mixup:
    ...
    def __call__(self, x, target):
        ...
        if len(target.shape) == 3:
            target = mixup_target_multilabel(target, 2, lam, self.label_smoothing)
        else:
            target = mixup_target(target, self.num_classes, lam, self.label_smoothing)
        return x, target


class MixupADDiag(Mixup):
    """Mixup/Cutmix that applies different params to each element or whole batch
    ...
    def __call__(self, x, target_ad, target_diag):
        assert len(x) % 2 == 0, "Batch size should be even when using this"
        if self.mode == "elem":
            lam = self._mix_elem(x)
        elif self.mode == "pair":
        ...
        target_ad = mixup_target(target_ad, 2, lam, self.label_smoothing)
        target_diag = mixup_target(target_diag, self.num_classes, lam, self.label_smoothing)
        return x, target_ad, target_diag


class FastCollateMixup(MixupADDiag):

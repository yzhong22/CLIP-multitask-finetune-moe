...
from models import build_moe_model
...
def get_args_parser():
    ...
    parser.add_argument("--freeze_experts", type=str2bool, default=True)
...
def main(args):
    ...
    dataset_test.set_classes(classes=dataset_train.classes, class_texts=dataset_train.class_texts)
    ...
    model = build_moe_model(args).to(device)
    model.init_text_features(dataset_train.class_texts)

    state_dicts = []
    for subset in args.subsets:
        ckpt = torch.load(os.path.join(args.expert_dir, f"train_{subset}", "checkpoint-best.pth"))
        state_dicts.append(ckpt["model"])
    model.load_experts(state_dicts)
...
if __name__ == "__main__":
    ...
    check_args(args)
    if args.output_dir:
        args.expert_dir = os.path.join(args.output_dir, args.backbone, f"seed{args.seed}", "lp")

        args.output_dir = os.path.join(
            args.output_dir, args.backbone, f"seed{args.seed}", "moe", f"train_{','.join(args.subsets)}"
        )
